...
const GET_ALL_COMPONENT_DETAIL_ERROR_DETAIL =
  'Error in getAllComponentDetailLikeVoltajeMaxEntradaService() function.';
//status
...
/**
 * @description get all paginated component details list according to its maximum input voltage from the database
 * @param {any} req any type
 * @param {any} res any type
 * @returns a json object with the transaction performed
 * @example
 */
const getAllComponentDetailLikeVoltajeMaxEntradaService = async (req) => {
  try {
    componentDetailList = null;
    voltajeMaxEntradaParam = null;
    queryStrParams = null;
    //Pagination
    pageSizeNro = 10;
    pageNro = 0;
    ...
    if (params != (null && undefined)) {
      voltajeMaxEntradaParam = params.voltajeMaxEntrada ? params.voltajeMaxEntrada : voltajeMaxEntradaParam;
    }
    //-- end with params  ---
    ...
    if (ComponentDetail != (null && undefined)) {
      await ComponentDetail.findAll({
        ...
        where: {
          voltaje_max_entrada: {
            [Op.iLike]: `%${voltajeMaxEntradaParam}%`,
          },
        },
        limit: pageSizeNro,
        offset: pageNro,
