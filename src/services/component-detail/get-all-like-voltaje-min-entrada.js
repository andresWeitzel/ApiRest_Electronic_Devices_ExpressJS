...
const GET_ALL_COMPONENT_DETAIL_ERROR_DETAIL =
  'Error in getAllComponentDetailLikeVoltajeMinEntradaService() function.';
//status
...
/**
 * @description get all paginated component details list according to its minimum input voltage from the database
 * @param {any} req any type
 * @param {any} res any type
 * @returns a json object with the transaction performed
 * @example
 */
const getAllComponentDetailLikeVoltajeMinEntradaService = async (req) => {
  try {
    componentDetailList = null;
    voltajeMinEntradaParam = null;
    queryStrParams = null;
    //Pagination
    pageSizeNro = 10;
    pageNro = 0;
    ...
    if (params != (null && undefined)) {
      voltajeMinEntradaParam = params.voltajeMinEntrada ? params.voltajeMinEntrada : voltajeMinEntradaParam;
    }
    //-- end with params  ---
    ...
    if (ComponentDetail != (null && undefined)) {
      await ComponentDetail.findAll({
        ...
        where: {
          voltaje_min_entrada: {
            [Op.iLike]: `%${voltajeMinEntradaParam}%`,
          },
        },
        limit: pageSizeNro,
        offset: pageNro,
