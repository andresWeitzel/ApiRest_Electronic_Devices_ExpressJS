...
const GET_ALL_COMPONENT_ERROR_DETAIL = 'Error in getAllComponentWithElectronicCapacitorService() function.';
...
/**
 * @description get all paginated components with electronic capacitors from the database
 * @param {any} req any type
 * @returns a json object with the transaction performed
 * @example
 */
const getAllComponentWithElectronicCapacitorService = async (req) => {
  try {
    componentList = null;
    queryStrParams = null;
    pageSizeNro = 10;
    ...
    if (Component != (null && undefined)) {
      await Component.findAll({
        ...
        where: {
          categoria: {
            [Op.iLike]: `%Capacitores Electroliticos%`,
          },
        },
        include: [
          { model: ElectrolycticCapacitor, required: true },
        ],
        limit: pageSizeNro,
        offset: pageNro,
        order: order,
