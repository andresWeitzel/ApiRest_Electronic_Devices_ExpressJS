...
const GET_ALL_MOSFET_TRANSISTOR_ERROR_DETAIL = 'Error in getAllMosfetTransistorController() function.';
 ...
 * @example
 */
const getAllMosfetTransistorController = async (req, res) => {
  try {
    msgResponse = null;
    msgLog = null;
    mosfetTransistors = await getAllMosfetTransistorService(req);
    ...
    switch (mosfetTransistors) {
      ...
      case null:
        res.status(BAD_REQUEST_CODE).send({
          error: GET_ALL_MOSFET_TRANSISTOR_BAD_REQUEST_DETAIL,
        });
        break;
      default:
