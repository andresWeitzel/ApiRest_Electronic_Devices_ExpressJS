...
//Services
const {
  getAllElectrolyticCapacitorLikeRangoTensionNominalService,
} = require('../../services/electrolytic-capacitor/get-all-like-rango-tension-nominal');
//Enums
...
const GET_ALL_ELECTROLYTIC_CAPACITOR_ERROR_DETAIL =
  'ERROR in getAllElectrolyticCapacitorLikeRangoTensionNominalController() function.';
const GET_ALL_ELECTROLYTIC_CAPACITOR_BAD_REQUEST_DETAIL =
  'Bad request, could not get electrolytic capacitors with the requested rango_tension_nominal.';
const GET_ALL_ELECTROLYTIC_CAPACITOR_NOT_FOUND_DETAIL =
  'No electrolytic capacitors found with the specified rango_tension_nominal.';
//Vars
let msgResponse;
let msgLog;

/**
 * @description get all electrolytic capacitors based on rango_tension_nominal from the database
 * @param {any} req any type
 * @param {any} res any type
 * @returns a json object with the transaction performed
 * @example
 */
const getAllElectrolyticCapacitorLikeRangoTensionNominalController = async (req, res) => {
  try {
    msgResponse = null;
    msgLog = null;

    electrolyticCapacitorList = await getAllElectrolyticCapacitorLikeRangoTensionNominalService(req);
