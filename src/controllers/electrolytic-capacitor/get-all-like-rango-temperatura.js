...
//Services
const {
  getAllElectrolyticCapacitorLikeRangoTemperaturaService,
} = require('../../services/electrolytic-capacitor/get-all-like-rango-temperatura');
//Enums
...
const GET_ALL_ELECTROLYTIC_CAPACITOR_ERROR_DETAIL =
  'ERROR in getAllElectrolyticCapacitorLikeRangoTemperaturaController() function.';
const GET_ALL_ELECTROLYTIC_CAPACITOR_BAD_REQUEST_DETAIL =
  'Bad request, could not get electrolytic capacitors with the requested rango_temperatura.';
const GET_ALL_ELECTROLYTIC_CAPACITOR_NOT_FOUND_DETAIL =
  'No electrolytic capacitors found with the specified rango_temperatura.';
//Vars
let msgResponse;
let msgLog;

/**
 * @description get all electrolytic capacitors based on rango_temperatura from the database
 * @param {any} req any type
 * @param {any} res any type
 * @returns a json object with the transaction performed
 * @example
 */
const getAllElectrolyticCapacitorLikeRangoTemperaturaController = async (req, res) => {
  try {
    msgResponse = null;
    msgLog = null;

    electrolyticCapacitorList = await getAllElectrolyticCapacitorLikeRangoTemperaturaService(req);
