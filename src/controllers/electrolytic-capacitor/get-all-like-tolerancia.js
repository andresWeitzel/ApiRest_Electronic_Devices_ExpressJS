...
//Services
const {
  getAllElectrolyticCapacitorLikeToleranciaService,
} = require('../../services/electrolytic-capacitor/get-all-like-tolerancia');
//Enums
...
const GET_ALL_ELECTROLYTIC_CAPACITOR_ERROR_DETAIL =
  'ERROR in getAllElectrolyticCapacitorLikeToleranciaController() function.';
const GET_ALL_ELECTROLYTIC_CAPACITOR_BAD_REQUEST_DETAIL =
  'Bad request, could not get electrolytic capacitors with the requested tolerancia.';
const GET_ALL_ELECTROLYTIC_CAPACITOR_NOT_FOUND_DETAIL =
  'No electrolytic capacitors found with the specified tolerancia.';
//Vars
let msgResponse;
let msgLog;

/**
 * @description get all electrolytic capacitors based on tolerancia from the database
 * @param {any} req any type
 * @param {any} res any type
 * @returns a json object with the transaction performed
 * @example
 */
const getAllElectrolyticCapacitorLikeToleranciaController = async (req, res) => {
  try {
    msgResponse = null;
    msgLog = null;

    electrolyticCapacitorList = await getAllElectrolyticCapacitorLikeToleranciaService(req);
