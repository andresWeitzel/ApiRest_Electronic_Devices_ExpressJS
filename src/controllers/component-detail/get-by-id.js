...
} = require('../../services/component-detail/get-by-id');
//Enums
...
const GET_BY_ID_COMPONENT_DETAIL_ERROR_DETAIL =
  'ERROR in getComponentDetailByIdController() function.';
const GET_BY_ID_COMPONENT_DETAIL_BAD_REQUEST_DETAIL =
  'Bad request, could not get a component detail with the requested id. Check if the component detail exist into the database.';
const GET_BY_ID_COMPONENT_DETAIL_NOT_FOUND_DETAIL =
...
/**
 * @description get a component detail based its id from the database
 * @param {any} req any type
 * @param {any} res any type
 * @returns a json object with the transaction performed
 * @example
 */
const getComponentDetailByIdController = async (req, res) => {
  try {
    msgResponse = null;
    msgLog = null;

    componentDetailList = await getComponentDetailByIdService(req);
    ...
    switch (componentDetailList) {
      ...
      case null:
        res.status(BAD_REQUEST_CODE).send({
          error: GET_BY_ID_COMPONENT_DETAIL_BAD_REQUEST_DETAIL,
        });
        break;
      default:
        if (
          (typeof componentDetailList === 'object' &&
            componentDetailList.hasOwnProperty('id')) ||
          ...
          res.status(OK_CODE).send({ ok: GET_BY_ID_COMPONENT_DETAIL_NOT_FOUND_DETAIL });
        } else {
